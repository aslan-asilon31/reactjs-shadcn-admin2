...
const CustomerEdit = () => {
  ...
  const deleteCustomer = useCustomerStore((state) => state.deleteCustomer); // Mengambil fungsi updateCustomer dari Zustand
  const updateCustomer = useCustomerStore((state) => state.updateCustomer); // Mengambil fungsi updateCustomer dari Zustand
  const fetchCustomerById = useCustomerStore((state) => state.fetchCustomerById); // Mengambil fungsi updateCustomer dari Zustand
  ...
  const fetchCustomerData = async () => {
    try {
      const customerData = await fetchCustomerById(id); // Mengambil data customer
      if (customerData) {
        setValue('name', customerData.name); // Mengisi form dengan data yang diambil
      }
    } catch (error) {
  ...
  useEffect(() => {
    fetchCustomerData(); // Memanggil fungsi untuk mengambil data saat komponen dimuat
  }, [id]);
  ...
  const onSubmit = async (data: Customer) => {
    ...
    try {
      await updateCustomer(data.name, generatedSlug, id); 
      loadingAlert.close();
